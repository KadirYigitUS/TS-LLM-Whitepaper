---
baslik: Kaynakça
---

# Kaynakça (Türkçe Özet)

````{tab-set}

```{tab-item} Türkçe

Türkçe özet aşağıdadır.

```

```{tab-item} English

:doc:`Go to the English reference list <en/references>`

```

````

```{note}
Kaynakça APA kurallarına göre İngilizce başlıklarla listelenir; Türkçe sayfa yalnızca özet sunar. DOI bulunmayan kitaplar için ISBN ve yayınevi bağlantıları verilir ve "DOI yok" ibaresi eklenir.
```

## Birincil Kaynaklar
- Vaswani, A., vd. (2017). *Attention Is All You Need*.
- Devlin, J., vd. (2019). *BERT*.
- Brown, T., vd. (2020). *Language Models are Few-Shot Learners*.
- Kaplan, J., vd. (2020). *Scaling Laws for Neural Language Models*.
- Wei, J., vd. (2022). *Chain-of-Thought Prompting*.
- Ouyang, L., vd. (2022). *InstructGPT*.
- Longpre, S., vd. (2023). *Flan Collection*.
- Touvron, H., vd. (2023). *LLaMA*.
- Schick, T., vd. (2023). *Toolformer*.

## Üçüncül Kaynaklar
- Reiss & Vermeer (2013). *Skopos kuramı*.
- Nord, C. (1997). *Amaçlı Çeviri*.
- Halliday, M. (1978). *Dil ve sosyal bağlam*.
- Bowker, L. (2023). *Çeviriyi Gizemden Arındırmak*.
- Kenny, D. (2022). *Herkes için Makine Çevirisi*.

Detaylı APA biçimi için :doc:`English References <en/references>` sayfasını kullanın.
