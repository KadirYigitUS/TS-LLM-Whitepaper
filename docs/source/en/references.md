---
title: References — APA Ordering
---

# References

## Primary Sources
- Brown, T., et al. (2020). *Language models are few-shot learners*. Advances in Neural Information Processing Systems, 33, 1877–1901. https://doi.org/10.48550/arXiv.2005.14165
- Devlin, J., Chang, M.-W., Lee, K., & Toutanova, K. (2019). *BERT: Pre-training of deep bidirectional transformers for language understanding*. NAACL, 1, 4171–4186. https://doi.org/10.48550/arXiv.1810.04805
- Kaplan, J., et al. (2020). *Scaling laws for neural language models*. arXiv. https://doi.org/10.48550/arXiv.2001.08361
- Longpre, S., et al. (2023). *The Flan collection: Designing data and methods for effective instruction tuning*. ICML 2023, 202, 22631–22648. https://doi.org/10.48550/arXiv.2301.13688
- Ouyang, L., et al. (2022). *Training language models to follow instructions with human feedback*. NeurIPS, 35, 27730–27744. https://doi.org/10.48550/arXiv.2203.02155
- Schick, T., et al. (2023). *Toolformer: Language models can teach themselves to use tools*. arXiv. https://doi.org/10.48550/arXiv.2302.04761
- Touvron, H., et al. (2023). *LLaMA: Open and efficient foundation language models*. arXiv. https://doi.org/10.48550/arXiv.2302.13971
- Vaswani, A., et al. (2017). *Attention is all you need*. Advances in Neural Information Processing Systems, 30. https://doi.org/10.48550/arXiv.1706.03762
- Wei, J., et al. (2022). *Chain-of-thought prompting elicits reasoning in large language models*. NeurIPS, 35, 24824–24837. https://doi.org/10.48550/arXiv.2201.11903

## Secondary Sources
- Jurafsky, D., & Martin, J. H. (2024). *Speech and language processing* (3rd draft). Stanford University. https://web.stanford.edu/~jurafsky/slp3/
- Mikolov, T., et al. (2013). *Distributed representations of words and phrases and their compositionality*. Advances in Neural Information Processing Systems, 26, 3111–3119. https://doi.org/10.48550/arXiv.1310.4546

## Tertiary Sources
- Bowker, L. (2023). *De-mystifying translation: Introducing translation to non-translators*. Routledge. https://doi.org/10.4324/9781003217718
- Halliday, M. A. K. (1978). *Language as social semiotic*. Edward Arnold.
- House, J. (2015). *Translation quality assessment: Past and present*. Routledge. https://doi.org/10.4324/9781315752839
- Kenny, D. (2022). *Machine translation for everyone*. Language Science Press. https://langsci-press.org/catalog/book/385
- Koehn, P. (2010). *Statistical machine translation*. Cambridge University Press. https://doi.org/10.1017/CBO9780511815820
- Koehn, P. (2020). *Neural machine translation*. Cambridge University Press. https://doi.org/10.1017/9781108608480
- Nord, C. (1997). *Translating as a purposeful activity*. St. Jerome.
- Reiss, K., & Vermeer, H. J. (2013). *Towards a general theory of translational action* (C. Nord, Trans.). St. Jerome.

```{note}
This list is synchronized with the Obsidian `References.md` file. Update both sources simultaneously to keep APA ordering consistent.
```
