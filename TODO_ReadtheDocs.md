...
| https://ts-llm-whitepaper.readthedocs.io/tr/latest/ | Form-wise / URL structure | WARN | Path returned 404; added .readthedocs.yaml prefix redirect so /tr/latest/* forwards to /en/latest/tr/*. | High | After creating ts-llm-whitepaper-tr RTD project, remove temporary redirect and enable official language switcher. |
...
| https://ts-llm-whitepaper.readthedocs.io/en/latest/ (site-wide) | Localization / Language switcher | WARN | RTD UI still shows only English; in-page tabs act as interim switcher. Redirects cover /tr/, but native locale selector remains absent. | High | Follow checklist in index.md (create Turkish project, link under Admin -> Translations). |
...
1. Stand up the `ts-llm-whitepaper-tr` project in Read the Docs, wire it under Admin -> Translations, and remove the temporary /tr redirect once live.
